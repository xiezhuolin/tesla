 ...
 */
public class ModifyDishCountAdp<T> extends BaseAdapter {
    public ModifyDishCountAdp(Context context) {
        super(context);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (dish.dishCount <= 10) {
            holder.tv_count.setTextColor(ContextCompat.getColor(context, R.color.red));
        } else {
            holder.tv_count.setTextColor(ContextCompat.getColor(context, R.color.green_press));
        }
        if(dish.getDishModifyCount() >=0)
        {
            holder.tv_danger.setTextColor(ContextCompat.getColor(context, R.color.green_press));
        }
        else{
            holder.tv_danger.setTextColor(ContextCompat.getColor(context, R.color.red));
        }
        holder.tv_dishName.setText((position + 1) + "." + dish.getDishName());
        holder.tv_danger.setText(dish.getDishModifyCount() + "/份");
        holder.tv_count.setText(dish.getDishCount() + "/份");
        //        }
        return convertView;
    }
