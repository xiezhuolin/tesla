...
import cn.acewill.pos.next.ui.adapter.StaffAdapter;
 ...
 */
public class StaffAty extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener, RefrushLisener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.aty_staff);
    ...
    private void initView() {
        myApplication.addPage(StaffAty.this);
//        setTitle("员工列表");
        //        setShowBtnBack(true);
        ...
        adapter = new StaffAdapter(context, this);
        ...
        orderLv.addOnItemTouchListener(new OnRecyclerItemClickListener(orderLv) {
            ...
            public void onItemClick(RecyclerView.ViewHolder viewHolder) {
                ...
                Staff staff = (Staff) adapter.getItem(position);
                if (staff != null) {
                    Intent intent = new Intent();
                    intent.setClass(StaffAty.this, StaffInfoAty.class);
    ...
    private List<Staff> getUserListForId(List<Staff> userList,String userId)
    {
        List<Staff> staffTemp = new CopyOnWriteArrayList<>();
        if(userList != null && userList.size() > 0)
        {
            for(Staff staff:userList)
            {
                if(staff != null)
                {
                    if(userId.equals(staff.getQuartersid()))
                    {
                        staffTemp.add(staff);
                    }
                }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.tv_login:
                startActivity(new Intent(this, StaffInfoAty.class));
                break;
    ...
    private List<Staff> userList = new CopyOnWriteArrayList<>();
    private void getUsers()
    {
        try {
            SystemService systemService = SystemService.getInstance();
            systemService.getStaff(new ResultCallback<List<Staff>>() {
                @Override
                public void onResult(List<Staff> result) {
                    if(result != null)
                    {
                        userList = ToolsUtils.cloneTo(result);
                ...
                public void onError(PosServiceException e) {
                    Log.i("获取员工列表失败,", e.getMessage());
                    showToast(ToolsUtils.returnXMLStr("get_staff_list_failure")+","+e.getMessage());
                }
            });
