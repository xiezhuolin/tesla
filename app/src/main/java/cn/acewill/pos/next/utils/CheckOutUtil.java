...
public class CheckOutUtil implements ScanGunKeyEventHelper.OnScanSuccessListener {
        ...
        if (orderItems != null) {
            for (OrderItem orderItem : orderItems) {
                if (!ToolsUtils.isList(orderItem.getTempMarketList())) {
                    orderItem.setMarketList(orderItem.getMarketList(), orderItem.getTempMarketList());
                }
    ...
    class InTask extends EPayTask {
        ...
        protected void onPostExecute(EPayResult result) {
            ...
            if (result != null && !result.success) {
                ...
                Log.e("正扫", "生成二维码失败");
            } else if (EPayResult.PAY_STATUS_COMPLETE.equalsIgnoreCase(result.trade_status)) {// 支付宝支付成功
            ...
            if (!TextUtils.isEmpty(values[0])) {
                ...
                if (store.isPadRunnIng()) {
                    ...
                    if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_7) {
                        SunmiSecondScreen.saveWxOrAliQcode(qrcode, payTypeID == 1 ? ToolsUtils.returnXMLStr("alipay") : ToolsUtils.returnXMLStr("wechat"), "￥" + total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
    ...
    class OutTask extends MicropayTask {
        ...
        protected void onPostExecute(EPayResult result) {
            ...
            } else if (result != null && result.weifutongPayStart)// 威富通成功
            {
                ...
                orderTradeNo = ToolsUtils.completionOrderId(orderId) + "_" + TimeUtil.getTimeToken();
                autoCode = result.code;
                getWayWeiFuTong(autoCode, ToolsUtils.handleCarDish(cart.getDishItemList()), totalFee, orderTradeNo);
            } else {
                if (pay_channel != PayReqModel.PTID_SSS_WEIXIN) {
                    showToast(ToolsUtils.returnXMLStr("pay_failure"));
                    Log.e("反扫", "支付失败");
    ...
    private void scanGunDialog() {
        //展示总金额到副屏
        if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_7) {
            SunmiSecondScreen.getInstance(context).sendData4DSD(payTypeID == 1 ? ToolsUtils.returnXMLStr("alipay") : ToolsUtils.returnXMLStr("wechat"), total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
        } else if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_14) {
    ...
    public void directlyCreateOrder(String tableName, BigDecimal money) {
        ...
        //7寸屏
        if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_7) {
            SunmiSecondScreen.getInstance(context).sendData4DSD(payment.getName() + ToolsUtils.returnXMLStr("pays"), total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
        } else if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_14) {
            SunmiSecondScreen.getInstance(context).showDishImgExcel(printOrder);
        }
        //        printMoney = money;
    ...
    private void payOrder(String tableName) {
        ...
        switch (payTypeID) {
            ...
            case 5://储值、积分、优惠券
                //                if (cart.getDishItemList() != null && cart.getDishItemList().size() > 0) {
                memberDialog = DialogUtil.memberDialog(context, payTypeID, total_money, printOrder.getItemList(), true, new CreatDealBack() {
                    @Override
            ...
            case -32://美团
                if (cart.getDishItemList() != null && cart.getDishItemList().size() > 0) {
                    DialogUtil.meiTuanDialog(context, posInfo.getOrderId(), total_money, true, null, new DialogMTCallback() {
                        @Override
...
//    }
            ...
            orderService.createCodeUrl(wftPayType, totalFee, orderTradeNo, new ResultCallback<WftRespOnse>() {
                ...
                public void onResult(WftRespOnse result) {
                    ...
                    if (result != null && !TextUtils.isEmpty(result.getCodeImgUrl())) {
                        ...
                        showToast(ToolsUtils.returnXMLStr("creat_qrcode_failure") + "!");
                        Log.e("正扫", "生成WFT二维码失败");
                    }
                }
                ...
                public void onError(PosServiceException e) {
                    progressDialog.disLoading();
                    showToast(ToolsUtils.returnXMLStr("creat_qrcode_failure") + "," + e.getMessage() + "!");
                    Log.e("正扫", "生成WFT二维码失败");
                }
            });
        } catch (PosServiceException e) {
    ...
    private void createWFTQrCode(String qrCode) {
        if (!TextUtils.isEmpty(qrCode)) {
            Bitmap qrcodeBit = CreateImage.convertStringToIcon(qrCode);
            //7寸屏
            if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_7) {
                SunmiSecondScreen.saveWxOrAliQcode(qrcodeBit, ToolsUtils.returnXMLStr("wft_pay"), "￥" + total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
            } else if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_14) {
                SunmiSecondScreen.showImgExcel(qrcodeBit, printOrder);
            }
            ...
            if (store.isPadRunnIng()) {
                paydialog = DialogUtil.scanPadDialog(context, payTypeID, task, qrCode, qrcodeBit,new InterfaceDialog() {
                    @Override
                    public void onCancle() {
                        Log.i("正扫手动下单：", "");
                        if (task != null)
    ...
    public void creatOrdAndCheckOut() {
        ToolsUtils.writeUserOperationRecords("TIME===>调用下单接口==" + TimeUtil.getStringTimeLong(System.currentTimeMillis()));
        if (posInfo.isMemberCheckOut()) {
            ...
            if (posInfo.isFreeOrder()) {
                ...
                printOrder.setAvtive_money(new BigDecimal(cart.getPriceSum()).setScale(2, BigDecimal.ROUND_DOWN));
                printOrder.setTake_money(new BigDecimal("0.00"));
            } else {
                printOrder.setCost(total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
                printOrder.setAvtive_money(new BigDecimal(cart.getPriceSum() - cart.getCost()).setScale(2, BigDecimal.ROUND_DOWN));
                printOrder.setTake_money(cart.getTakeMoney());
                if (anyCheckoutPayMent != null) {
                    printOrder.setCost(printMoney.setScale(2, BigDecimal.ROUND_DOWN).toString());//注意,因为是允许输入任意金额支付所以这里需要把cost改为任意输入的金额
                }
            }
    ...
    private void creatPaymaxCharge() {
        ...
        try {
            ...
            storeBusinessService.creatPaymaxCharge(printMoney.toString(), subject, ToolsUtils.handleCarDish(cart.getDishItemList()), orderId, "wechat_csb", "192.168.1.66", "789456", new ResultCallback<LKLResponse>() {
                @Override
    ...
    private void createQrCode(String chargeid, String qrCode) {
        if (!TextUtils.isEmpty(qrCode)) {
            ...
            //7寸屏
            if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_7) {
                SunmiSecondScreen.saveWxOrAliQcode(qrcode, ToolsUtils.returnXMLStr("lkl_pay"), "￥" + total_money.setScale(2, BigDecimal.ROUND_DOWN).toString());
            } else if (SunmiSecondScreen.getDeviceType() == SunmiSecondScreen.SCRENN_14) {
                SunmiSecondScreen.showImgExcel(qrcode, printOrder);
            }
